...
public static class BiasGameStatEmbedProcessor
{
    public static Embed[] CreateEmbed(string nickName, GenderEnum gender, UserBiasGameStatResource stats)
    {
        ...
        _ = builder.WithTitle($"Bias Game Stats for {nickName}");
        _ = builder.WithDescription(
        ...
        _ = builder.WithThumbnailUrl(stats.Stats.First().LatestImageUrl);
        _ = builder.WithCurrentTimestamp();
        _ = builder.WithFooter($"Gender: {gender.ToDatabaseFriendlyString()}");
        ...
        _ = builder.AddField($"Your Favorites from *{stats.BiasGameCount} games*", list);
