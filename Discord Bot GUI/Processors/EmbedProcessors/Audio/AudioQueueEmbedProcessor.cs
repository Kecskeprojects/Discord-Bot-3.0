...
public static class AudioQueueEmbedProcessor
{
    public static Embed[] CreateEmbed(ServerAudioResource audioResource, int index)
    {
        ...
        _ = builder.WithTitle($"Queue (page {index} of {Math.Ceiling((songcount - 1) / 10.0)}):");
        ...
        for (int i = 0; i < audioResource.MusicRequests.Count; i++)
        {
            ...
            if (i == 0)
            {
                _ = builder.AddField("\u200b", $"__Currently Playing:__\n**[{item.Title}]({item.URL})**\nRequested by:  {item.User}", false);
                _ = builder.WithThumbnailUrl(item.Thumbnail);
            }
            ...
            {
                _ = builder.AddField("\u200b", $"**{i}. [{item.Title}]({item.URL})**\nRequested by:  {item.User}", false);
            }
        ...
        _ = builder.AddField("Full duration:", "" + (hour > 0 ? hour + "h" : "") + minute + "m" + second + "s", true);
